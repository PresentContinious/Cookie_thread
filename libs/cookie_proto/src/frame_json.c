...
int cookie_frame_to_json(const struct sensor_frame *f, char *buf, size_t buf_len)
{
	...
	if (f->pad_len > 0) {
		/* Packet-size sweep filler. Emitted in 32-byte chunks so the
		 * pad length is free of format-string limits. */
		static const char fill[] = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";

		APPEND(",\"pad\":\"");
		for (size_t left = f->pad_len; left > 0;) {
			size_t chunk = MIN(left, sizeof(fill) - 1);

			APPEND("%.*s", (int)chunk, fill);
			left -= chunk;
		}
		APPEND("\"");
	}
