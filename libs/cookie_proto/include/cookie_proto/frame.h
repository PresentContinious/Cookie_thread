...
struct sensor_frame {
	...
	/* Measurement padding for the packet-size sweep: when > 0 the encoder
	 * appends "pad":"xxx..." of exactly pad_len characters, growing the
	 * payload so 6LoWPAN fragments the frame into several link frames.
	 * Consumers ignore the key (unknown-key skip). */
	uint16_t pad_len;
};
