...
void cookie_thread_format_src(char src_out[5])
{
	...
	/* Use the Thread extended (MAC) address, not the factory EUI-64: this is
	 * the identity a router reports for us in its neighbour table, so the
	 * gateway's topology tree lines up with this src field. */
	const otExtAddress *ext = otLinkGetExtendedAddress(inst);
	snprintf(src_out, 5, "%02x%02x", ext->m8[6], ext->m8[7]);
}
