...
static void report_work_handler(struct k_work *w)
{
	...
	/* Phase markers own the LED pads during the timing campaign (same
	 * semantics as the SED loop and the RIOT/Contiki-NG ports): red = cycle
	 * start to transmit handed off, green = sensor window. The role-blink
	 * and attached-LED diagnostics are suppressed so they do not pollute
	 * the marker edges. */
	if (led_red_lp.port) {
		gpio_pin_set_dt(&led_red_lp, 1);
	}
...
#if defined(CONFIG_NODE_MEAS_LED)
	if (led_grn_lp.port) {
		gpio_pin_set_dt(&led_grn_lp, 0);
	}
#endif

	/* Local telemetry print for single-board bring-up: emit the same JSON
	 * line the gateway would, so one board shows its own readings over the
	 * console (RTT or UART) without a separate gateway present. */
	{
		char json[CONFIG_COOKIE_PROTO_FRAME_BUF_SIZE];
		int jn = cookie_frame_to_json(&f, json, sizeof(json));
		if (jn > 0) {
			printk("%s\n", json);
		}
	}
...
#if defined(CONFIG_NODE_MEAS_LED)
	/* Falling edge: transmit handed off. Red width = cycle latency. */
	if (led_red_lp.port) {
		gpio_pin_set_dt(&led_red_lp, 0);
	}
#endif
