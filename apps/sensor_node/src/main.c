...
int main(void)
{
	...
	/* Measurement aid: five green blinks at boot confirm the image is
	 * running, since the SED loop otherwise drives no LED. */
	for (int i = 0; i < 5 && led_green.port; i++) {
	...
	/* Persistent OT state is kept across reboots so the node remembers the
	 * mesh and rejoins quickly, which is what makes partition merge and
	 * self-healing work as described in the Thread spec. A one-time clean
	 * wipe is available behind CONFIG_COOKIE_FACTORY_RESET_ON_BOOT for
	 * clearing stale datasets from earlier experiments. */
