...
class MeshState:
    ...
    def render_topology_ascii(self) -> str:
        ...
        out: list[str] = []
        for root in roots:
            self._draw_subtree(root, "", True, out, is_root=True)
        return "\n".join(out)
    ...
    def _draw_subtree(self, node: str, prefix: str, is_last: bool, out: list[str],
                      is_root: bool = False) -> None:
        connector = "└── " if is_last else "├── "
        role = self.nodes.get(node, NodeRow(src=node)).role
        if is_root and role == "?":
            # The tree comes from the collector, which reports its children but
            # sends no telemetry of its own, so no frame ever carries its role.
            # The static snapshot view already labels it this way.
            role = "GATEWAY"
