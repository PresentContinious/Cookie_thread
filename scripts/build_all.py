...
VARIANTS = [
    ...
    # --- Cookie over its own USB-C (Nordic Open Bootloader) -----------------
    # Same three images linked at 0x1000, above the bootloader's static MBR,
    # with the console on USB-CDC: a board updated over USB is also read over
    # USB, so no nRF52-DK is in the path at all. Package the .hex with
    # scripts/make_dfu_zips.ps1 before handing it to nrfutil.
    {
        "name":    "cookie_gateway_dfu",
        "app":     APPS / "gateway",
        "board":   "cookie_nrf_v200/nrf52840",
        "extra":   ["usb_console.conf", "dfu.conf"],
        "overlay": ["usb_console.overlay", "dfu.overlay"],
        "comment": "Cookie gateway, DFU image for the Open Bootloader.",
    },
    {
        "name":    "cookie_node_auto_dfu",
        "app":     APPS / "sensor_node",
        "board":   "cookie_nrf_v200/nrf52840",
        "extra":   ["overlays/profile_auto.conf", "overlays/usb_console.conf",
                    "overlays/dfu.conf"],
        "overlay": ["overlays/usb_console.overlay", "overlays/dfu.overlay"],
        "comment": "Cookie sensor node, AUTO profile, DFU image.",
    },
    {
        "name":    "cookie_node_sed_dfu",
        "app":     APPS / "sensor_node",
        "board":   "cookie_nrf_v200/nrf52840",
        "extra":   ["overlays/profile_sed.conf", "overlays/usb_console.conf",
                    "overlays/dfu.conf"],
        "overlay": ["overlays/usb_console.overlay", "overlays/dfu.overlay"],
        "comment": "Cookie sensor node, SED profile, DFU image.",
    },
...
def build_one(v, build_root: Path, pristine: bool, prefix, west_cmd) -> int:
    ...
    # Zephyr takes both lists semicolon-separated and applies them in order,
    # so a later fragment overrides an earlier one (dfu.conf last).
    if v["extra"]:
        cmake_args.append("-DEXTRA_CONF_FILE=" + ";".join(v["extra"]))
    if v.get("overlay"):
        cmake_args.append("-DEXTRA_DTC_OVERLAY_FILE=" + ";".join(v["overlay"]))
